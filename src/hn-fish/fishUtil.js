...
export let timeToTraverseScreen = Math.max(
    (getOceanWidth() + 275) / 0.04, // Fish body offset or something like that
    2 * 40000 // The magic
)
...
export function initializeFish(f) {
    ...
    const getInitialXPosition = () =>
        f.xDirection > 0 ? -1 * f.width : getOceanWidth()
    ...
    const targetXPosition =
        f.xDirection > 0 ? getOceanWidth() : -1 * maxFishWidth
...
export function getXPositionAtTime(f, time) {
    const progress =
        (time - f.xStartTime) / (timeToTraverseScreen * f.speedModifier)
    const position =
        f.getInitialXPosition() +
        (f.targetXPosition - f.getInitialXPosition()) * progress
...
function getThetaAtTime(fish, time) {
    const progress =
        (time - fish.yStartTime) / (fish.speedModifier * timeToTraverseScreen)
...
function fishSine(f, theta) {
    return (
        -1 *
            f.amplitude *
            Math.sin(theta * f.phase * periodModifier + f.phaseShift) +
        getYBaselineInPx(f)
    )
...
export function shiftFishAnimationOrigin(fish, dx, dy) {
    ...
    fish.yBaseline = yPxToBaseline(fish, getYBaselineInPx(fish) + dy)
}
...
export function setFishPhase(fish, targetPhase, time = performance.now()) {
    ...
    fish.phaseShift =
        ((fish.phaseShift + phaseDeltaRad) * fish.phase) % (2 * Math.PI)
}
